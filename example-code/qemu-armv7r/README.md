...
early-access 'experimental' *Armv7-R bare-metal* (`armv7r-none-eabihf`) target.
...
virtual machine that is emulating an Arm cortex-r5f system. Both demos:

1. Print "Hello, world!" to the PL011 UART0, which is typically
...
file configures the default target as `armv7r-none-eabihf`. It also sets up the
...
The compiled outputs will go into `./target/armv7r-none-eabihf/<profile>`, where
`<profile>` is `debug` or `release`. The binary is called `qemu-armv7r`, because
that's the name given in the `Cargo.toml` file.

```console
$ criticalup run cargo run --release -bin no_heap
   Compiling qemu-armv7r v0.1.0 (/Users/jonathan/work/qemu-armv7r)
    Finished release [optimized] target(s) in 0.16s
     Running `qemu-system-arm -machine versatileab -cpu cortex-r5f -semihosting -nographic -kernel target/armv7r-none-eabihf/release/no_heap`
Hello, this is Rust!
