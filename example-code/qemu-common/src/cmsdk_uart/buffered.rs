...
impl<const QLEN: usize> BufferedUart<QLEN> {
    ...
    pub fn flush(&self) {
        ...
        while self.with(|inner| inner.uart.tx().tx_fifo_full()) {
            core::hint::spin_loop();
        }
    }
    ...
    pub fn rx_isr(&self) {
        defmt::debug!("- RX ISR");
        self.with(|inner| {
            let rx = inner.uart.rx();
            if rx.interrupt_status() {
                rx.clear_interrupts();
